...
# If feasible, extract results and validate
if model.status == 1:
    results = []
    for year in demand['Year'].unique():
        for v_id in vehicles['ID']:
            if use_vars[(year, v_id)].varValue is not None and use_vars[(year, v_id)].varValue > 0:
                results.append([year, v_id, int(use_vars[(year, v_id)].varValue)])
    results_df = pd.DataFrame(results, columns=['Year', 'ID', 'Num_Vehicles'])
    print(results_df)
    ...
    for _, row in demand.iterrows():
        ...
        total_met = sum(
            use_vars[(year, v_id)].varValue * vehicles.loc[vehicles['ID'] == v_id, 'Yearly range (km)'].values[0]
            for v_id in vehicles['ID']
            if (year, v_id) in use_vars and
            vehicles.loc[vehicles['ID'] == v_id, 'Size'].values[0] == size and 
            vehicles.loc[vehicles['ID'] == v_id, 'Distance'].values[0] >= distance
        )
        
        #print(f"Year: {year}, Size: {size}, Distance: {distance}, Demand: {total_demand}, Met: {total_met}")
        assert total_met >= total_demand, f"Demand not met for Year: {year}, Size: {size}, Distance: {distance}"
...
for year in range(2023, 2039):
    for v_id in vehicles['ID']:
        ...
        purchase_year = int(v_id.split('_')[-1])
        vehicle_info = vehicles[vehicles['ID'] == v_id]
        if vehicle_info.empty:
            print(f"Warning: Vehicle ID {v_id} not found in vehicles DataFrame")
            continue
        size = vehicle_info['Size'].values[0]
        distance_bucket = vehicle_info['Distance'].values[0]
        # Buy operations
        ...
        if (year, v_id) in use_vars:
            ...
            if num_used > 0:
                ...
                yearly_range = vehicle_info['Yearly range (km)'].values[0]
...
# Insert the correction code here
import itertools

# Create all possible combinations of years, sizes, and distances based on demand.csv
demand_combinations = demand[['Year', 'Size', 'Distance']].drop_duplicates()
...
# Add missing combinations with zero entries
missing_combinations = []
for _, row in demand_combinations.iterrows():
    year, size, distance = row['Year'], row['Size'], row['Distance']
    matching_rows = results_df[(results_df['Year'] == year) & 
                               (results_df['Distance_bucket'] == distance) & 
                               (results_df['ID'].str.contains(f'_{size}_'))]
    
    if matching_rows.empty:
        matching_vehicle = vehicles[(vehicles['Size'] == size) & (vehicles['Distance'] == distance)]
        if not matching_vehicle.empty:
            vehicle_id = matching_vehicle['ID'].values[0]
            missing_combinations.append({
                'Year': year,
                'ID': vehicle_id,
                'Num_Vehicles': 0,
                'Type': 'Use',
                'Fuel': vehicles_fuels[vehicles_fuels['ID'] == vehicle_id]['Fuel'].values[0],
                'Distance_bucket': distance,
                'Distance_per_vehicle(km)': 0.0  # Ensure this is a float
            })

# Use pd.concat() to add the missing combinations
if missing_combinations:
    results_df = pd.concat([results_df, pd.DataFrame(missing_combinations)], ignore_index=True)



# Remove zero entries before final validation
#results_df = results_df[results_df['Num_Vehicles'] > 0]



# Remove Placeholder entries before final submission
#results_df = results_df[~results_df['ID'].str.startswith('Placeholder')]
...
def validate_and_correct_submission(submission_df, vehicles, vehicles_fuels, fuels):
    ...
    if not all(submission_df['Distance_per_vehicle(km)'].apply(lambda x: isinstance(x, float))):
        errors.append(7)
    ...
    # Allow zero vehicles to be present in the final results
    ...
    for _, row in submission_df.iterrows():
        if row['Type'] == 'Use':
            year = row['Year']
            vehicle_info = vehicles[vehicles['ID'] == row['ID']]
            if vehicle_info.empty:
                print(f"Warning: Vehicle ID {row['ID']} not found in vehicles DataFrame")
                continue
            size = vehicle_info['Size'].values[0]
    ...
    for _, row in submission_df[submission_df['Type'] == 'Use'].iterrows():
        v_id = row['ID']
        vehicle_fuel_info = vehicles_fuels[vehicles_fuels['ID'] == v_id]
        if vehicle_fuel_info.empty:
            print(f"Warning: Vehicle ID {v_id} not found in vehicles_fuels DataFrame")
            errors.append(23)
            continue
        correct_fuel = vehicle_fuel_info['Fuel'].values[0]
...
results_df.to_csv(os.path.join(results_dir, 'submission_1.csv'), index=False)
